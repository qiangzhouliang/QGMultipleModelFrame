 ...
 */
class AutoLoginPresenterImpl(var mContext: Context?, var loginView: BaseView<LoginModel>?) : BasPresenter,
    ...
    override fun loadDatas(map:HashMap<String,String?>?) {
        //1定义一个request+执行
        val mRequest = MRequest(mContext, url = ConstantUrl.Auth.AUTO_LOGIN,handler = this,
            reqMap = map,isShowProgress = false,turnsType = LoginModel::class.java)
        BaseRequest<LoginModel>().postRequest(mRequest)
    }
