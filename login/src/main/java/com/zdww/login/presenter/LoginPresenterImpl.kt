 ...
 */
class LoginPresenterImpl(var mContext: Context?, var loginView: BaseView<LoginModel>?) : BasPresenter,
    ...
    override fun loadDatas(map:HashMap<String,String?>?) {
        //1定义一个request+执行
        val mRequest = MRequest(mContext, url = ConstantUrl.Auth.LOGIN,handler = this,
            reqMap = map,loadMessage = "正在登录，请稍后......",turnsType = LoginModel::class.java)
        BaseRequest<LoginModel>(0).postRequest(mRequest)
    }
